...
static void OOD(benchmark::State& inState)
{
    ...
    for (unsigned int boat_index = 0; boat_index < num_of_boats; ++boat_index)
    {
        ...
        ({ 
            ...
            RandFloat(20.0f, 100.0f),                       // Maintanance cost per rent;
    ...
    for (auto _ : inState)
    {
        ...
        for (unsigned int boat_index = 0; boat_index < num_of_boats; ++boat_index)
        {
            const float earnings = boats_info[boat_index].CalculateEarning();
            const float cost = boats_info[boat_index].CalculateMaintenanceCost() +
                               boats_info[boat_index].CalculateCrewCost();

            average_return_on_investment_ptc += (earnings / cost) * 100.0f - 100.0f;
